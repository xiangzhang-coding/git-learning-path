...
この課の練手区では、`/origin` がそのリモートリポジトリの場所——ローカルの `/repo` とは独立したメモリ上のリポジトリ——です。**リモートには `cd` で入れません**：そこには履歴しかなく、ワークツリーがありません（実際のベアリポジトリやサーバー上のリポジトリと同じです）。あなたが操作するのはローカルのこのコピーで、git コマンドを通じてデータをやり取りします。
