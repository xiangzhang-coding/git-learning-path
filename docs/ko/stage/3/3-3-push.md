...
push는 본질적으로 원격 브랜치를 로컬 브랜치의 위치로 **패스트포워드**시키는 일입니다(패스트포워드 개념은 챕터 2의 merge에서 나왔습니다). 푸시에 성공하면 로컬의 추적 브랜치 `origin/main`도 함께 전진합니다. 이것은「원격이 지금 어디 있는지」의 미러이므로 이제 원격과 일치하게 됩니다.

**upstream(상류)**: 푸시에 성공하면 로컬 브랜치와 원격 브랜치 사이에 상류·하류 관계가 생깁니다. 원격 브랜치가 로컬 브랜치의 upstream이 되는 것입니다. 이후 `git push` / `git pull`은 인자 없이도 어느 원격 브랜치와 동기화해야 하는지 알 수 있습니다.
