...
push 本质上是把远程分支**快进**到本地分支的位置（快进的概念来自章节 2 的 merge）。推送成功后，本地的跟踪分支 `origin/main` 也会同步前进——它是「远程此刻在哪」的镜像，现在与远程一致。

**upstream（上游）**：推送成功后，本地分支与远程分支建立上下游关系——远程分支是本地分支的 upstream。之后 `git push` / `git pull` 不带参数也知道该和哪个远程分支同步。
