...
По сути push — это **fast-forward** ветки remote до положения локальной ветки (понятие fast-forward — из главы 2, merge). После успешного push локальная tracking branch `origin/main` тоже продвигается — она зеркалит «где сейчас remote» и теперь совпадает с ним.

**upstream (вышестоящий)**: после успешного push между локальной веткой и веткой remote устанавливается связь «вышестоящий — нижестоящий»: ветка remote становится upstream локальной ветки. После этого `git push` / `git pull` без аргументов знают, с какой веткой remote синхронизироваться.
