...
**Older syntax**: `git checkout <name>` and `git checkout -b <name>` are the legacy commands that do the same thing; `git switch` is the newer recommended one, and the playground supports both. `git checkout` also has a "restore files" use, now covered by `git restore` (Stage 1).
